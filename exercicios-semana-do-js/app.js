...
const getOddNumbers = item % 2 === 1;

const oddNumbers = randomNumbers.filter((item) => getOddNumbers);
...
const countLowerThanFiveHundredOne = (acc, num) =>
  num < 501 ? ++acc : acc;
...
const lowerThanFiveHundredOne = crazyNumbers.reduce(countLowerThanFiveHundredOne, 0);

console.log(lowerThanFiveHundredOne);
...
const filteredMovies = tarantinoMovies.filter(movie => movie.release < 2000);
...
const filteredTvShows = tvShows.map(serie => {
...
/*
Se a partir de um array queremos gerar um valor string, é um caso de uso para reduce
  - Nome 1
  - Nome 2
  - Nome 3
*/

const listCart = cart.reduce((acc, { name }) =>
  `${acc}- ${name}\n`, '');

// const listCart = cart.filter(cartItem => {
